...
<script type="text/javascript">
	...
	function sum()
	{
		...
		// 『GET』: 데이터 전송 및 페이지 요청 방식(POST 사용 가능. 단, POST 사용 시 헤더 추가)
		// 『url』: AJAX 요청 페이지(데이터 수신 및 응답 페이지)
		// 『true』: boolean → true(비동기), false(동기). 동기는 사용하지 안함.
		ajax.open("GET", url, true);
		//-- 실제 서버와의 연결이 아니고, 일종의 환경설정 부분으로 이해하는 것이 좋다.
		//   (실제 서버 연결은 나중에...)
		
		// ajax 객체(XMLHttpRequest 객체)의 readyState 속성은
		// 각 단계별로 변화되며, 0 부터 4 까지 순차적으로 증가한다.
		// 그때마다... 『readystatechange』 이벤트가 발생한다.
		ajax.onreadystatechange = function()
		{
			...
			if (ajax.readyState == 4)
			{
				...
				// 그 과정에서 데이터가 올바른 것인지 에러 메시지를 받은 것인지 알 수 없기 때문에
				// 서버로부터 응답 코드를 확인한 후 업무를 진행하도록 처리(구성)한다.
				if (ajax.status == 200)
				{
					// 업무 진행	
					//-- 업무 코드를 외부로 추출
...
<input type="button" value=" = " class="short" onclick="sum()" />
