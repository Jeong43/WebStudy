...
<!-- 
		※ 요쳥한 페이지(jsptest003.jsp)의 결과를 확인한 후 새로고침을 반복해보면...
		   ...
		   ==> 변수 a는 클래스(jsptest_jsp)의 전역변수로 선언된 것을 확인할 수 있다.
		       이는 객체가 소멸되기 전까지 계속 값이 유지된다는 것을 의미한다.
		       ...
		       이로 인해 변수 b는 페이지에 대한 요청이 발생할 때마다
		       ...
		       서블릿으로 변환되는 과정에서 메소드 내부에 또 다른 메소드가 정의되는 상황이 되어버리기 때문에
		       문법적으로 잘못된 표현이다.
